...
브랜드명만 랭킹화하면 `나이키`, `아식스`처럼 너무 넓은 결과가 나오므로, 상품/모델 단위 후보를 뽑는 옵션을 추가했습니다.

```bash
python sourcing_ranker.py --input "브랜드&키워드 300개.xlsx" --top 100 --discover-products --products-only --product-seed-limit 60 --product-keywords-per-brand 8 --min-product-volume 1000 --max-product-candidates 200
```

결과 엑셀의 `상품키워드_TOP` 시트와 `reports/product_keywords_YYYYMMDD.csv`를 확인하세요.
